...
def upload_db_returning(db_fragment, table, class_id):
    ...
    with get_connection() as con:
        ...
        if db_fragment["slug"] in slugs:
            ...
            element_id = dict(cursor.fetchone())["id"]
        else:
            cursor.execute(
...
def download_all_slugs(table):
    with get_connection() as con:
        ...
        slugs = [dict(i)["slug"] for i in cursor.fetchall()]
...
def check_for_valid(form, table, slugs=None, dublicate=True):
    ...
    if slugs is None:
        with get_connection() as con:
            cursor = con.cursor()
            cursor.execute(f"SELECT slug FROM {table}")
            slugs = [dict(row)["slug"] for row in cursor.fetchall()]
...
def add_narrative():
    with get_connection() as con:
        ...
        categories = [dict(row)["category"] for row in cursor.fetchall()]
...
def confirm_result():
    ...
    with get_connection() as con:
        cursor = con.cursor()
        cursor.execute(
            f"INSERT INTO classes ({keys}) VALUES ({placeholders});",
...
def export_png():
    ...
    full_url = f"http://127.0.0.1:8001{relative_url}"
